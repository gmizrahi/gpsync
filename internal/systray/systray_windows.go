...
type winTray struct {
	...
	// dblClickMs/suppressNextUp implement double-click detection on top of
	// Shell_NotifyIcon's legacy per-click message forwarding, which sends
	// WM_LBUTTONUP after EVERY click including the one that turns out to
	// be the second half of a double-click (sequence for a genuine
	// double-click: ...UP, DBLCLK, UP). Without this, a double-click fired
	// showMenu() TWICE in quick succession, and the popup menu rendered
	// underneath the taskbar -- the z-order race two overlapping
	// TrackPopupMenu calls produce. Fixed by never acting on a click
	// immediately: the first
	// UP arms a one-shot WM_TIMER for the system's real double-click
	// interval instead of opening the menu right away; if a DBLCLK
	// follows before that timer fires, the timer is killed and
	// suppressNextUp is set so the double-click's own trailing UP is
	// ignored instead of arming a second timer. Only ever touched from
	// wndProc, which always runs on this single locked OS thread (see
	// init()'s runtime.LockOSThread()), so no synchronization needed.
	dblClickMs     uint32
	suppressNextUp bool
}

// idDblClickTimer is the WM_TIMER id used for the pending-single-click
// delay described above. Only one such timer is ever outstanding (a new
// UP always starts by killing/replacing it), so a single fixed id is fine.
const idDblClickTimer = 1
...
func (t *winTray) wndProc(hWnd windows.Handle, message uint32, wParam, lParam uintptr) (lResult uintptr) {
	...
	case WM_ENDSESSION:
		...
		systrayExit()
	case WM_TIMER:
		if wParam == idDblClickTimer {
			// The delayed single click's timer fired -- no WM_LBUTTONDBLCLK
			// arrived to cancel it within the system's real double-click
			// interval, so this genuinely was just one click.
			pKillTimer.Call(uintptr(t.window), idDblClickTimer)
			t.showMenu()
		}
	case t.wmSystrayMessage:
		switch lParam {
		case WM_LBUTTONDBLCLK:
			// A double-click opens the dashboard. Cancel the pending
			// single-click timer (if
			// the first click's UP already armed one) so the menu never
			// opens for what's actually a double-click, and suppress the
			// UP that Windows always sends right after this message so IT
			// doesn't arm a fresh timer either. fireOnDoubleClick runs the
			// callback on its own goroutine, matching how onReady is
			// invoked, so a slow handler (opening a browser) never blocks
			// this message pump.
			pKillTimer.Call(uintptr(t.window), idDblClickTimer)
			t.suppressNextUp = true
			fireOnDoubleClick()
		case WM_LBUTTONUP:
			if t.suppressNextUp {
				// The trailing UP after a WM_LBUTTONDBLCLK -- already
				// handled, not a new click.
				t.suppressNextUp = false
				return
			}
			// Don't open the menu immediately: this click might be the
			// first half of a double-click, which arrives as its own
			// WM_LBUTTONDBLCLK shortly after. Wait out the system's real
			// double-click interval first; see dblClickMs/suppressNextUp's
			// doc comment on winTray for the full sequence this resolves.
			ms := t.dblClickMs
			if ms == 0 {
				ms = 500
			}
			pSetTimer.Call(uintptr(t.window), idDblClickTimer, uintptr(ms), 0)
		case WM_RBUTTONUP:
			// Right-click doesn't participate in double-click detection --
			// Shell_NotifyIcon never sends WM_RBUTTONDBLCLK through this
			// legacy forwarding path, so there's nothing to wait out.
			t.showMenu()
		}
...
func (t *winTray) initInstance() error {
	...
	dblClickRes, _, _ := pGetDoubleClickTime.Call()
	t.dblClickMs = uint32(dblClickRes)
...
func registerSystray() {
	if err := wt.initInstance(); err != nil {
		log.Printf("systray: unable to init instance: %v", err)
		return
	}

	if err := wt.createMenu(); err != nil {
		log.Printf("systray: unable to create menu: %v", err)
		return
	}
...
func nativeLoop() {
	...
	for {
		...
		switch int32(ret) {
		case -1:
			log.Printf("systray: error at message loop: %v", err)
			return
		case 0:
			return
		default:
...
// RemoveIcon deletes the tray icon immediately via Shell_NotifyIcon
// (NIM_DELETE), synchronously -- unlike Quit(), which only posts WM_CLOSE
// for the message loop to process later (see quit()/wndProc's WM_DESTROY
// case, where the real NIM_DELETE normally happens). A caller about to
// os.Exit() right away, without ever giving that message loop another
// chance to run, needs this instead: without it, the process dies with
// the icon's Shell_NotifyIcon entry never deleted, and Windows leaves it
// sitting in the notification area -- looking alive -- until Explorer
// happens to notice the owning window is gone, typically only on the
// next mouse hover over it -- so without this the icon lingers after the
// process quits until the pointer passes over it.
func RemoveIcon() {
	wt.muNID.Lock()
...
func SetIcon(iconBytes []byte) {
	...
	if err != nil {
		log.Printf("systray: unable to write icon data to temp file: %v", err)
		return
	}
	if err := wt.setIcon(iconFilePath); err != nil {
		log.Printf("systray: unable to set icon: %v", err)
		return
	}
}
...
func (item *MenuItem) SetIcon(iconBytes []byte) {
	...
	if err != nil {
		log.Printf("systray: unable to load icon from temp file: %v", err)
		return
	}

	h, err = wt.iconToBitmap(h)
	if err != nil {
		log.Printf("systray: unable to convert icon to bitmap: %v", err)
		return
	}
	...
	if err != nil {
		log.Printf("systray: unable to addOrUpdateMenuItem: %v", err)
		return
	}
}
...
func SetTooltip(tooltip string) {
	if err := wt.setTooltip(tooltip); err != nil {
		log.Printf("systray: unable to set tooltip: %v", err)
		return
	}
}
...
func addSeparator(id uint32) {
	...
	if err != nil {
		log.Printf("systray: unable to addSeparator: %v", err)
		return
	}
}
...
func hideMenuItem(item *MenuItem) {
	...
	if err != nil {
		log.Printf("systray: unable to hideMenuItem: %v", err)
		return
	}
}
